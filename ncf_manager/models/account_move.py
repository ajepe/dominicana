    ...
    def validate_fiscal_purchase(self):
        ...
        if NCF and self.journal_id.purchase_type == "normal":
            if NCF[-10:-8] == "02" or NCF[1:3] == "32":
                ...
                raise ValidationError(
                    _(
                        "Proveedor sin RNC/Céd\n\n"
                        "El proveedor *{}* no tiene RNC o Cédula y es requerido "
                        "para registrar compras Fiscales"
                ...
                raise ValidationError(
                    _(
                        "NCF NO pasó validación en DGII\n\n"
                        "¡El número de comprobante *{}* del proveedor "
                        "*{}* no pasó la validación en "
                        "DGII! Verifique que el NCF y el RNC del "
                        "proveedor estén correctamente "
                        "digitados, o si los números de ese NCF se "
    ...
    def action_invoice_open(self):
        for inv in self:
            if inv.amount_total == 0:
                raise UserError(
                    _(
                        "No se puede validar una factura cuyo monto total sea"
                        " igual a 0."
                    )
                )
            ...
            if inv.move_type == "out_invoice" and inv.journal_id.ncf_control:
                if not inv.partner_id.sale_fiscal_type:
                    raise ValidationError(
                        _(
                            "El cliente [{}]{} no tiene Tipo de comprobante, y es"
                ...
                if sequence.number_next_actual > sequence.max_number_next:
                    raise ValidationError(
                        _(
                            "Los comprobantes para {} se han agotado,"
                ...
                if (
                    ...
                    raise UserError(
                        _(
                            "El cliente [{}]{} no tiene RNC/Céd, y es requerido"
                ...
                if (
                    ...
                    raise UserError(
                        _(
                            "Si el monto es mayor a RD$250,000 el cliente debe "
                ...
                if inv.reference and inv.journal_id.purchase_type in (
                    ...
                    if not inv.partner_id.vat:
                        raise ValidationError(
                            _(
                                "¡Para este tipo de Compra el Proveedor"
                                " debe de tener un RNC/Cédula/NIT establecido!"
                            )
                        )
                    ...
                    if (
                        ...
                        raise ValidationError(
                            _(
                                "¡Para Remesas al Exterior el Proveedor debe"
                                " tener país diferente a República Dominicana!"
                            )
                        )
